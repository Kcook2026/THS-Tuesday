...
Deno.serve(async (req) => {
  try {
    ...
    const { formId, values, fileUrls } = body;
    ...
    if (form.status !== 'active') return Response.json({ error: 'Form is not active' }, { status: 400 });
    ...
    for (const field of fields) {
      ...
      // For file fields, derive display value from file URLs
    ...
    // Build WorkboardItem data with default title
    const itemData = {
      workspace: form.workspace,
      workboard: form.workboard,
    ...
    // Assign to default group (first non-archived)
    ...
    // Create WorkboardItem
    const item = await base44.asServiceRole.entities.WorkboardItem.create(itemData);
    ...
    if (itemData.owner) recipients.add(itemData.owner);
    ...
    for (const recipientId of recipients) {
      ...
      await base44.asServiceRole.entities.Notification.create({
        ...
        message: `${user.full_name || user.email} submitted "${form.title}", creating item "${itemData.title}"`,
        record_type: 'FormSubmission',
        record_id: submission.id,
        target_url: `/workboards/${form.workboard}`,
      }).catch(() => {});
    }
